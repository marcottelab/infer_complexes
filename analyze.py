...
def arrfeats_prep_all_data(arrfeats, ppis, sp='Hs', cutoff=0.5):
    print "Adding species summary."
    arrfeats = fe.arr_add_spsummary(arrfeats, cutoff)
    print "Adding ppis."
    arrfeats = fe.arrfeats_add_ppis(arrfeats, ppis)
    _,_,all_cxs = ppi.load_training_complexes(sp, None,'') 
    pdgold = pd.PairDict(co.pairs_from_complexes(dict(all_cxs)))
    print "Setting trues."
